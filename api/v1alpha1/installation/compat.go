...
// per known breaking change between components, keyed by molding kind.
...
// CheckCompatibility resolves every molding's version from its image (the source
// of truth) and validates the combination against the compatibility matrix.
func (c *Casting) CheckCompatibility(logger *slog.Logger) error {
