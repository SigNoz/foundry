...
Deploys a SigNoz [VM Collection Agent](https://signoz.io/docs/opentelemetry-collection-agents/vm/overview/) on a Red Hat, CentOS, or Fedora host, on top of the OpenTelemetry Collector Contrib `.rpm` package. The package owns the `otelcol-contrib` binary and its systemd unit; foundry's managed surface is a systemd drop-in (`otelcol-contrib.service.d/foundry.conf`) plus the collector config at `/etc/otelcol-contrib/config.yaml`. The agent collects the host's telemetry and exports it, along with anything your applications send it, to any SigNoz: Self-Hosted Community, Self-Hosted Enterprise, or SigNoz Cloud.
...
- A Red Hat, CentOS, Fedora (or compatible) host with `systemd`
- The OpenTelemetry Collector Contrib `.rpm` package installed:
...
Removes foundry's drop-in first, then stops and disables `otelcol-contrib.service` (its daemon-reload picks up the removal). `/etc/otelcol-contrib/config.yaml` stays: it is a package conffile the `.rpm` shipped, not something foundry recorded, so foundry cannot restore it on melt. The package itself is never uninstalled; use `rpm -e otelcol-contrib` to remove it.
