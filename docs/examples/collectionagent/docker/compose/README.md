...
The generated collector config follows the [Docker Collection Agent configuration guide](https://signoz.io/docs/opentelemetry-collection-agents/docker/configure/) and exports to `${env:SIGNOZ_INGESTION_ENDPOINT}`.

### Point the agent at your SigNoz

Set the endpoint and environment through `spec.collector.spec.env`, which becomes the container's environment:
