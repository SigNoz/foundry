...
func (enricher *linuxMoldingEnricher) EnrichStatus(ctx context.Context, kind v1alpha1.MoldingKind, config *v1alpha1.Casting) error {
	...
	case v1alpha1.MoldingKindTelemetryStore:
		...
		if replicas > 1 || shards > 1 {
			return fmt.Errorf("deployment mode '%s' does not support Distributed Clickhouse Setup, raise an issue at https://github.com/signoz/foundry/issues to request support for this mode", config.Spec.Deployment.Mode)
		}
