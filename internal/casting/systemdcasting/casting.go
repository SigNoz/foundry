...
func (l *linuxCasting) Forge(ctx context.Context, cfg v1alpha1.Casting) ([]types.Material, error) {
	...
	for _, tmpl := range l.castings {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to forge %s: %w", tmpl.String(), err)
		}
...
func (l *linuxCasting) forgeIngester(tmpl *types.Template, cfg *v1alpha1.Casting) ([]types.Material, error) {
	...
	spec.Status.Extras["cfgPath"] = ingestermolding.IngesterConfigFileName(name, "", 0)
	spec.Status.Extras["cfgOpampPath"] = ingestermolding.IngesterOpampFileName(name, "", 0)

	m, err := l.materialzedTemplate(tmpl, cfg, name+"-ingester"+svcSuffix)
	return []types.Material{m}, err
}
...
func (l *linuxCasting) forgeTelemetryStore(tmpl *types.Template, cfg *v1alpha1.Casting) ([]types.Material, error) {
	...
	mats, err := l.configMaterials(spec.Spec.Config.Data)
	if err != nil {
		return nil, err
	}
	...
	for s := 0; s < shards; s++ {
		for r := 0; r < reps; r++ {
			spec.Status.Extras["cfgPath"] = telemetrystoremolding.StoreInstanceConfigFileName(name, kind, s, r)
...
func (l *linuxCasting) forgeTelemetryKeeper(tmpl *types.Template, cfg *v1alpha1.Casting) ([]types.Material, error) {
	...
	for r := 0; r < reps; r++ {
		spec.Status.Extras["cfgPath"] = telemetrykeepermolding.KeeperConfigFileName(name, kind, r)
