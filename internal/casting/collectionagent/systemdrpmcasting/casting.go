...
type systemdRPMCasting struct {
	logger *slog.Logger
}

func New(logger *slog.Logger) *systemdRPMCasting {
	return &systemdRPMCasting{logger: logger}
}

func (c *systemdRPMCasting) Enricher(ctx context.Context, config *collectionagent.Casting) (collectionagentmolding.MoldingEnricher, error) {
	return newSystemdRPMMoldingEnricher(), nil
}

func (c *systemdRPMCasting) Forge(ctx context.Context, config collectionagent.Casting, p *pourer.Pourer) error {
...
func (c *systemdRPMCasting) Cast(ctx context.Context, config collectionagent.Casting, outputPath string, p *pourer.Pourer, toolers []tooler.Tooler) error {
	...
	if _, err := systemd.Cat(ctx, systemdtooler.Options{Unit: unitName}); err != nil {
		return foundryerrors.Wrapf(err, foundryerrors.TypeNotFound, "failed to find the %q unit: install the package first with `sudo rpm -ivh otelcol-contrib_<version>_linux_<arch>.rpm`", unitName)
	}
...
func (c *systemdRPMCasting) Melt(ctx context.Context, config collectionagent.Casting, outputPath string, p *pourer.Pourer, toolers []tooler.Tooler) error {
	systemd, err := systemdtooler.Lookup(toolers)
	if err != nil {
		return err
	}
