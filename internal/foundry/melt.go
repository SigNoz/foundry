...
// Melt melts every document of the set, against the order the lock
...
func (foundry *Foundry) Melt(ctx context.Context, machineries []v1alpha1.Machinery, poursPath string) error {
	...
	for _, p := range slices.Backward(planners) {
		...
		if err := p.Melt(ctx, poursPath); err != nil {
			return err
		}
	}
