...
func NewConnection(address domain.Address, ca []byte, tokens oauth2.TokenSource) (Connection, error) {
	if address.Host() == "" {
		return Connection{}, errors.Newf(errors.TypeInvalidInput, "failed to create connection: no address is stated")
	}

	if len(ca) == 0 {
		return Connection{}, errors.Newf(errors.TypeInvalidInput, "failed to create connection: no certificate authority is stated")
	}

	if tokens == nil {
		return Connection{}, errors.Newf(errors.TypeInvalidInput, "failed to create connection: no token source is stated")
	}
