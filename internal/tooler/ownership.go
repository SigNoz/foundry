...
func Verify(ctx context.Context, tool Tool, release domain.Release, list func(context.Context) (domain.Ownership, error)) error {
	...
	if err != nil {
		tool.Logger.WarnContext(ctx, "skipping the ownership check: could not read labels", slog.String("tool", tool.Name()), foundryerrors.LogAttr(err))

		return nil
	}

	if foreign, conflict := ownership.Foreign(release.Owner); conflict {
		return foundryerrors.Newf(foundryerrors.TypeInvalidInput, "failed to run %s: %q already belongs to [%s], not [%s]: remove it, or deploy under a different name", tool.Name(), release.Name, foreign, release.Owner)
	}
